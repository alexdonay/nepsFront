...
export default function LocationsList() {
  ...
  const loadLocations = async () => {
    try {
      const { data } = await api.get('/cadastros/locations');
      setLocations(data);
    } catch (e) {
      setLocations([]);
    }
  ...
  const handleSave = async () => {
    try {
      if (editId) {
        await api.put(`/cadastros/locations/${editId}`, form);
      } else {
        await api.post('/cadastros/locations', form);
      }
      setShowDialog(false);
      setForm({ name: '', health_unit_id: null, morning_slots: 0, afternoon_slots: 0, evening_slots: 0 });
      setEditId(null);
      loadLocations();
    } catch (err) {
  ...
  return (
    <div className="surface-card p-4 shadow-2 border-round">
      <div className="flex justify-content-between mb-3">
        ...
        <Button label="Novo Local" icon="pi pi-plus" onClick={() => { setEditId(null); setForm({ name: '', health_unit_id: null, morning_slots: 0, afternoon_slots: 0, evening_slots: 0 }); setShowDialog(true); }} />
      </div>
      ...
      <Dialog visible={showDialog} onHide={() => setShowDialog(false)} header={editId ? 'Editar Local' : 'Novo Local'}>
        ...
        <div className="field mb-3">
          ...
          <Dropdown value={form.health_unit_id} options={healthUnits} optionLabel="name" optionValue="id" onChange={(e) => setForm({...form, health_unit_id: e.value})} className="w-full" placeholder="Selecione" />
        </div>
        <div className="grid">
          <div className="col-4">
            <label>Vagas Manhã</label>
            <InputNumber value={form.morning_slots} onChange={(e) => setForm({...form, morning_slots: e.value})} className="w-full" />
          </div>
          <div className="col-4">
            <label>Vagas Tarde</label>
            <InputNumber value={form.afternoon_slots} onChange={(e) => setForm({...form, afternoon_slots: e.value})} className="w-full" />
          </div>
          <div className="col-4">
            <label>Vagas Noite</label>
            <InputNumber value={form.evening_slots} onChange={(e) => setForm({...form, evening_slots: e.value})} className="w-full" />
          </div>
        </div>
