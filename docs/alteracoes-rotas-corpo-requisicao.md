...
- **Rotas de listagem voltaram a receber filtros via URL (`query params`) no `GET`.**
...
| Cursos | `POST /v1/disciplines/detail` | `{ "discipline_id": <id> }` |
...
| Unidades | `POST /v1/internships/detail` | `{ "internship_id": <id> }` |
...
Os filtros nas chamadas de listagem devem ser enviados via URL (query string).

Exemplo:

```http
GET /v1/users?page=1&per_page=10&name_like=abc
```
...
- `POST /v1/rooms/by-service` com `{ "internship_id": <id> }`
- `POST /v1/students/by-discipline` com `{ "discipline_id": <id> }`
